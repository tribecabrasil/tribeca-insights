...
def test_setup_environment_sets_ssl_cert_file(caplog):
    ...
    setup_environment()
    assert os.environ['SSL_CERT_FILE'] == certifi.where(), "SSL_CERT_FILE not set to certifi bundle"
    assert any("Using certifi CA bundle for SSL" in record.message for record in caplog.records), "Expected log message not found"
...
def test_get_stopwords_caches_and_downloads(monkeypatch):
    ...
    assert not called.get('downloaded', False), "Stopwords should not be downloaded if already present"
...
def test_safe_strip():
    ...
    assert safe_strip("  hello ") == "hello", "safe_strip did not trim whitespace correctly"
    assert safe_strip(None) == "", "safe_strip did not return empty string for None input"
    assert safe_strip(123) == "", "safe_strip did not return empty string for non-string input"
